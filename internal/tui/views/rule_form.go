...
// celVars returns the CEL variable names available for a given resource type,
// used to drive autocomplete in the When field.
...
// NewRuleForm creates a rule builder form. If existing is non-nil, fields are
// pre-populated for editing; onSave receives the assembled RuleConfig once it
// passes validation, and onCancel is called if the user cancels.
func NewRuleForm(_ *tview.Application, readonly bool, existing *rules.RuleConfig, onSave func(rules.RuleConfig), onCancel func()) tview.Primitive {
	form := tview.NewForm()
	...
	// Track the When text separately so callbacks can reference it before the
	// form item exists.
	whenExpr := whenVal
	...
	form.AddDropDown("Resource", resources, resourceIdx, func(option string, _ int) {
	...
	whenField.SetAutocompletedFunc(func(text string, _, source int) bool {
		whenExpr = text
	...
	form.AddButton("Save", func() {
		...
		cooldown := form.GetFormItemByLabel("Cooldown").(*tview.InputField).GetText()
		if cooldown != "" {
		...
		dryRun := form.GetFormItemByLabel("Dry Run").(*tview.Checkbox).IsChecked()
		if readonly {
			dryRun = true
		}

		onSave(rules.RuleConfig{
			Name:     name,
			Enabled:  &enabled,
			DryRun:   dryRun,
